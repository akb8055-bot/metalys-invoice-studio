...
const storedRecords = () => JSON.parse(localStorage.getItem(recordsKey) || '[]').map((record) => record.type === 'proforma' ? { ...record, status: 'Approved' } : record)
...
const fresh = (type = 'proforma') => ({ id: id(), type, number: nextNumber(type), date: today(), poNumber: '', place: 'Doha, Qatar', status: type === 'invoice' ? 'Pending' : 'Approved', customerName: '', customerAddress: '', customerTaxId: '', items: [item()], discount: 0, paymentTerms: 'Payment due within 30 days', notes: type === 'invoice' ? 'Thank you for your business.' : 'This proforma invoice is valid for 30 days.' })
let state = fresh()
...
const totals = () => {
  ...
  const tax = state.type === 'invoice' ? state.items.reduce((sum, row) => sum + row.quantity * row.rate * row.tax / 100, 0) : 0
...
const documentTotal = (record) => {
  ...
  const tax = record.type === 'invoice' ? rows.reduce((sum, row) => sum + Number(row.quantity || 0) * Number(row.rate || 0) * Number(row.tax || 0) / 100, 0) : 0
...
function renderItems() {
  ...
  document.querySelector('#items-head').innerHTML = `<th>Description</th><th>Qty</th><th>Unit</th><th>Rate (QAR)</th>${invoice ? '<th>Tax %</th>' : ''}<th>Amount</th><th></th>`
  document.querySelector('#items').innerHTML = state.items.map((row, index) => `<tr data-index="${index}"><td><textarea data-item="description" rows="1" placeholder="Material or service">${escape(row.description)}</textarea></td><td><input data-item="quantity" type="number" min="0" step="0.01" value="${row.quantity}"></td><td><input data-item="unit" value="${escape(row.unit)}"></td><td><input data-item="rate" type="number" min="0" step="0.01" value="${row.rate}"></td>${invoice ? `<td><input data-item="tax" type="number" min="0" step="0.01" value="${row.tax}"></td>` : ''}<td class="row-total">${money(row.quantity * row.rate * (invoice ? 1 + row.tax / 100 : 1))}</td><td><button class="remove" title="Remove item" ${state.items.length === 1 ? 'disabled' : ''}><i data-lucide="trash-2"></i></button></td></tr>`).join('')
  createIcons({ icons })
}
...
function renderPaper() {
  ...
  if (!invoice) {
    document.querySelectorAll('.paper-table tr').forEach((row) => row.children[5]?.remove())
    document.querySelector('.terms > div')?.remove()
    ;[...document.querySelectorAll('.summary p')].find((row) => row.querySelector('span')?.textContent === 'Tax')?.remove()
    ;[...document.querySelectorAll('.parties p')].find((row) => row.textContent.startsWith('Tax ID / CR:'))?.remove()
  }
...
function populate() {
  if (state.type === 'proforma') {
    state.status = 'Approved'
    state.items.forEach((row) => { row.tax = 0 })
  }
  const statuses = state.type === 'invoice' ? ['Pending', 'Paid', 'Overdue'] : ['Approved']
  document.querySelector('#status').innerHTML = statuses.map((status) => `<option>${status}</option>`).join('')
  document.querySelector('#status').disabled = state.type === 'proforma'
  document.querySelector('#customer-tax-field').hidden = state.type === 'proforma'
...
function renderHistory() {
  ...
  document.querySelector('#history-list').innerHTML = filtered.length ? filtered.map((record) => {
    const statuses = record.type === 'invoice' ? ['Pending', 'Paid', 'Overdue'] : ['Approved']
...
document.querySelector('#items').addEventListener('input', (event) => { const row = event.target.closest('tr'); const key = event.target.dataset.item; if (!row || !key) return; state.items[row.dataset.index][key] = ['quantity', 'rate', 'tax'].includes(key) ? Number(event.target.value) : event.target.value; row.querySelector('.row-total').textContent = money(state.items[row.dataset.index].quantity * state.items[row.dataset.index].rate * (state.type === 'invoice' ? 1 + state.items[row.dataset.index].tax / 100 : 1)); renderPaper() })
